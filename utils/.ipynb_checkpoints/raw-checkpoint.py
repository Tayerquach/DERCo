...
def convert_data_to_BIDS(raw_data, subject_id, task):
    """
    ...
    output_path = ('BIDS')# define output path

    events_data, events_id = mne.events_from_annotations(raw_data,event_id='auto')#find events from annotations
    bids_basename = BIDSPath(subject=subject_id,task=task, root = output_path)#make bids name
