"""Module contains frequently used functions dealing with raw data."""
import os
...
from mne_bids import write_raw_bids, BIDSPath #make_bids_basename

from .config import settings


def read_raw_data(raw_file_path: str, **kwargs):
    """
...
def convert_data_to_BIDS(raw_data, subject_id, topic_id, task):
    """
    Convert raw EEG to BIDS format
    Parameters
    ----------
    raw_data: Raw instance
    subject_id: selected participant's index
    task: task's name (e.g., reading, listening ...)
    -------
    Returns
    None
    -------
    ...
    BIDS folder
    """
    os.makedirs(f'BIDS/{subject_id}',exist_ok=True)
    output_path = (f'BIDS/{subject_id}')# define output path

    events_data, events_id = mne.events_from_annotations(raw_data,event_id='auto')#find events from annotations
    bids_basename = BIDSPath(subject=topic_id,task=task, root = output_path)#make bids name
    write_raw_bids(raw_data, bids_path=bids_basename, event_id=events_id,events_data=events_data,overwrite=True)#convert to BIDS
...
def filter_raw(raw: Raw, n_jobs, **kwargs) -> Raw:
    """
    ...
    raw_filter = (
        ...
        .copy()
        .notch_filter(
            freqs=settings["notch_filter"]["line_freq"],
            n_jobs=n_jobs,
            **kwargs,
            )
        .filter(
...
def filter_raw_line_freq(raw: Raw, **kwargs) -> Raw:
    """
    Applies band-pass filtering to Raw instance.
    Parameters
    ----------
